...
public class AuthServiceImpl implements IAuthService {
    ...
    final SimpleGrantedAuthority simpleGrantedAuthorityAdmin = new SimpleGrantedAuthority("ROLE_ADMIN");
    ...
    @com.example.dacn.config.annotations.BlackListToken
    public LoginResponse login(LoginDto loginDto) {
        ...
        boolean rfConThoiHanVaKoKhoa = rfRp.isPresent() && new Date(rfRp.get().getExprired())
                .after(new Date(System.currentTimeMillis())) &&
                //doạn này, nếu token trong black list đã bị khóa thì trả về true mà đã bị khóa thì cần phải
                //tạo token mới, nên phải đảo ngược lại để toán tử là false để tạo token mới.
                !blackListTokenRepo.tokenDaBiKhoaHayChua(tk.getId(), rfRp.get().getToken());

        String rf = rfConThoiHanVaKoKhoa ?
                rfRp.get().getToken() :
                insertRefreshToken(jwtSevice.createRefreshToken(tk), tk),
                accessToken = buildLoginToken(tk);
        var resp = LoginResponse.builder()
                .token(accessToken)
                .refreshToken(rf)
                .taiCheToken(rfConThoiHanVaKoKhoa)
                .tenTaiKhoan(tk.getUsername())
                .build();
        resp.setIdLog(tk.getId());
        return resp;
    }
    ...
    @LogAction(action = EnumBehavior.UPDATE, bangTuongTac = "nguoi_dung, role", descriptions = "Phân quyền người dùng")
    public Set<TaiKhoanResponese> phanQuyenTaiKhoan(Set<PhanQuyenRq> quyenRq) {
        ...
        //Lấy hết id trong rq để tìm hết đỡ phải dùng vòng lặp
        ...
        boolean managerOrAdmin = flatRole.contains(EnumRole.ADMIN) || flatRole.contains(EnumRole.MANAGER),
                coPhaiAdmin = authentication.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toSet()).contains("ROLE_ADMIN");

        if (managerOrAdmin && !coPhaiAdmin) {
            throw new AccessDeniedException("Bạn không có quyền phân quyền cao hơn");
        }

        if (!coPhaiAdmin && roleRepo.kiemTraDSIdCoNguoiDungAdmin(uuids)) {
            throw new AccessDeniedException("Trong danh sách phân quyền có admin");
        }
//        Map<EnumRole, Set<UUID>> mapIdByEnumRole = quyenRq.stream().flatMap(phanQuyenRq ->
//                phanQuyenRq.getRoles().stream().map(role-> Map.entry(role, phanQuyenRq.getId()))
//        ).collect(Collectors.groupingBy(Map.Entry::getKey,Collectors.mapping(Map.Entry::getValue, Collectors.toSet())));


        TaiKhoan tkHienTai = ((TaiKhoan) authentication.getPrincipal());
        Map<UUID, TaiKhoan> userMap = getDsTaiKhoanResponese(tkHienTai.getId(), uuids, coPhaiAdmin);
    ...
    private Map<UUID, TaiKhoan> getDsTaiKhoanResponese(UUID idUser, Set<UUID> uuids, boolean isAdmin) {
        if (isAdmin) {
            return taikhoanRepo.findAllById(uuids).stream().filter(item
                            -> //Bỏ qua các người dùng role admin tránh trường hợp giảm quyền người dùng admin
                            !item.getAuthorities().contains(simpleGrantedAuthorityAdmin))
                    .collect(Collectors.toMap(TaiKhoan::getId, u -> u));
        } else {
            return taikhoanRepo.findAllById(uuids).stream().filter(item
                            -> //Bỏ qua các người dùng role admin tránh trường hợp giảm quyền người dùng admin và id quản lí phải là người đang đăng nhập
                            !item.getAuthorities().contains(simpleGrantedAuthorityAdmin) && item.getManager() != null && item.getManager().getId().equals(idUser))
                    .collect(Collectors.toMap(TaiKhoan::getId, u -> u));
        }
    }
            ...
            try {
                ...
                if (typeMap == null) {
                    ...
                    typeMap.addMappings(mapping -> {
                        ...
                        mapping.map(TaiKhoan::getId, TaiKhoanResponese::setIdLog);
                        mapping.map((taiKhoan) -> taiKhoan.getManager().getId(), TaiKhoanResponese::setManager);
                    });
    ...
    private String insertRefreshToken(String token, TaiKhoan idTaiKhoan) {
        return rfTkRp.save(new RefreshToken(idTaiKhoan, jwtSevice.exprired(token).toInstant(ZoneOffset.UTC).toEpochMilli(), token)).getToken();
    }
