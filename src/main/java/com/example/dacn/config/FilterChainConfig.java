...
public class FilterChainConfig {
	...
	public FilterChainConfig(Environment environment, ObjectMapper objectMapper) {
		...
		this.path = environment.matchesProfiles("dev")
				? new String[] { "/swagger-ui/**", "/swagger-ui.html",
	...
	SecurityFilterChain filterChainSpam(HttpSecurity http) throws Exception {
		http.securityMatcher("/*/public/*")
				.addFilterAt(new SpamFilter(), UsernamePasswordAuthenticationFilter.class)
				.csrf(CsrfConfigurer::disable).cors(CorsConfigurer::disable)
				.authorizeHttpRequests(rq -> rq.anyRequest().permitAll())
				.headers(header -> header.xssProtection(xss -> xss.headerValue(
	...
	SecurityFilterChain filterChain(HttpSecurity httpSecurity,
		...
		httpSecurity
				...
				.authorizeHttpRequests(rq -> {
					rq.requestMatchers("/store/*").hasAnyAuthority("SYSTEM_ADMIN")
							.requestMatchers("/product/*")
							.hasAnyAuthority("SYSTEM_ADMIN", "STORE_OWNER", "STAFF")
							.requestMatchers(path).permitAll().anyRequest()
							.authenticated();
				})
	...
	private void accessDeniedHandle(HttpServletRequest httpServletRequest,
	...
	private void authenticationEntryPoint(@NonNull HttpServletRequest httpServletRequest,
			@NonNull HttpServletResponse httpServletResponse, AuthenticationException e)
			throws IOException {
		...
		if (pro != null) {
			httpServletResponse.getWriter()
					.write(this.objectMapper.writeValueAsString(pro));
		} else {
			Map<String, Object> map = Map.of("message", "User not authorized",
