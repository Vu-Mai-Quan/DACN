...
@RestController
@RequestMapping("/auth/")
@Tag(name = "Authen", description = "Bảo mật")
...
public class AuthController {
	...
	public ResponseEntity<Object> login(@RequestBody @Valid LoginDto param) {
		...
		ResponseCookie refreshToken = ResponseCookie
				.from(REFRESH.name(), resLogin.refreshToken()).sameSite("lax")
				.httpOnly(true).path("/").secure(true)
				.maxAge(Duration.ofMinutes(this.timeAgeRefresh)).build();
		return ResponseEntity.ok().header(HttpHeaders.SET_COOKIE, refreshToken.toString())
				.body(Map.of("tokenAccess", resLogin.accessToken()));
	}
